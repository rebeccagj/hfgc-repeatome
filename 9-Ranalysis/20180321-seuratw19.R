...
w19 = FilterCells(object = w19, subset.names = c("nGene", "percentmito"), low.thresholds = c(200, -Inf), high.thresholds = c(Inf, 0.06))
...
w19 <- RunTSNE(object = w19, dims.use = 1:9, do.fast = T)
TSNEPlot(object = w19, colors.use = clustercolors)
...
scattercolors = c("lightgrey", "darkblue")
...
makegraphs = function(genelist){
  ...
  VlnPlot(object = w19, features.plot = c(genelist), cols.use = clustercolors)
}
...
makegraphs(c("DNMT3A","PIWIL4","EZH2")) #stephen's genes